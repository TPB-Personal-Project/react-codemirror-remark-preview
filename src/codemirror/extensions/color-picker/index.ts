...
const hexStringRegex = /#(?:[0-9a-f]{3}){1,2}/igm

const combinedRegex = new RegExp(`(${rgbCallExpRegex.source})|(${hexStringRegex.source})`)
...
function colorPickersDecorations(view: EditorView) {
    ...
    for (const range of view.visibleRanges) {
        syntaxTree(view.state).iterate({
            ...
            to: range.to,
            enter: ({type, from, to}) => {
                //console.log("Node type is " + type.name)

                if (type.name === 'Paragraph') {
                    const text = view.state.doc.sliceString(from, to);
                    console.log("Matching against string " + text)
                    const matches = text.match(hexStringRegex);
                    if(!matches) return;
                    ...
                    for (let [index, value] of matches.entries()) {
                        ...
                        //const value = Object.values(match.groups!!).join();
                        console.log("Value " + value)
                        const [color, alpha] = toFullHex(value);
                        console.log("Split into " + color + " and " + alpha)
                        offset += index;
                        const fromIndex = from + offset;
                        offset += value.length;
                        const toIndex = from + offset;

                        console.log(`Indices are from ${fromIndex} to ${toIndex}`);
                        ...
                        widgets.push(widget.range(fromIndex));
                    }
                    ...
                    //     const toIndex = from + i + value.length;
                    //
                    //     const widget = Decoration.widget({
                    //         widget: new ColorPickerWidget({
                    //             colorType: ColorType.hex,
